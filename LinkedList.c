...
void insertMiddleNode(linkedList_h* L, listNode* pre, char* x) {
	...
	if (L->head == NULL) {		//공백 리스트인 경우
		newNode->link = NULL;	//새 노드를 첫 번째 shemfh
		L->head = newNode;
	}
...
void reverse(linkedList_h* L) {
	...
	//리스트의 첫 번째 노드부터 링크를 따라 다음 노드로 이동하면서 노드 간 연결 바꿈
	while (p != NULL) {
		r = q;
		q = p;
		p = p->link;
